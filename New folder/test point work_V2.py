# -*- coding: utf-8 -*-
"""
Created on Mon Mar  8 17:43:42 2021
...
class MY_GUI():
    ...
    def openfile(self):
        ...
        table, _, _,_,_ = findresult(l, p, e, Q, bq, partnu, s3, file[0])

        tab = table.copy(deep=False)

        for i in file:
            check, injQ, Backflow, sss,injname = findresult(l, p, e, Q, bq, partnu, s3, i)
       ...
       # line2_data  = {'Max': ['|'], 'Min': ['|'],'average':['|']}
        ...
        result_a = pd.concat([data_a_q, data_a_b,data_a_s], axis=0, ignore_index=True)
        self.log_data_Text.delete(1.0,END)
        self.log_data_Text.insert(1.0,result_a.T)
        
        result_i = pd.concat([injection_q, backflow_q,S3S_q], axis=0, ignore_index=True)
        self.init_data_Text.delete(1.0,END)
        self.init_data_Text.insert(1.0,result_i.T)
        
        self.asd = pd.concat([tab,result_a,result_i], axis=1).T
    ...
    def savefile(self):
        try:
            ...
            self.asd.to_excel('result_V1.xlsx')
        except:
            self.result_data_Text.delete(1.0, END)
            self.state_data_Text.delete(1.0, END)
...
def findresult(l, p, e, Q, bq, partnu, s3, i):
    ...
    table_head2 = pd.concat([table_head,  table_head, table_head], axis=0, ignore_index=True)
    ...
    return table_head2, injQ, Backflow, sss, injname
